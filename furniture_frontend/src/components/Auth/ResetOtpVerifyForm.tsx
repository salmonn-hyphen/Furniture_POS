...
export default function ResetOtpVerifyForm({
  className,
  ...props
}: React.ComponentProps<"div">) {
  const submit = useSubmit();
  const navigation = useNavigation();
  const form = useForm<z.infer<typeof ResetOtpSchema>>({
  ...
  function onSubmit(values: z.infer<typeof ResetOtpSchema>) {
    submit(values, { method: "POST", action: "/reset/verify" });
  }
  return (
    <div className={cn("w-full px-3 sm:px-4", className)} {...props}>
      <Card className="mx-auto w-full max-w-sm rounded-xl md:max-w-sm lg:max-w-md">
        <CardHeader className="space-y-1 pb-3 text-center md:pb-4">
          <CardTitle>Verify your OTP for Reset</CardTitle>
          <CardDescription>
          ...
          <Field className="space-y-3">
            ...
            <Form {...form}>
              ...
              >
                ...
                >
                  {navigation.state === "submitting" ? <Spinner /> : "Verify"}
                </Button>
              </form>
            </Form>
